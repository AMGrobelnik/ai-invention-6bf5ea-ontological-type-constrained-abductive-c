...
\begin{enumerate}
  \item \textbf{ROCStories} (1000 examples): Narrative stories where the last sentence must be abduced from previous context. Each example provides 4 sentences as input and the 5th sentence as output. Domain: narrative. Task type: abductive.

  \item \textbf{RuleTaker} (15000 examples): Logical entailment tasks with contextual facts and questions. Each example provides a context (set of facts) and a question, with a binary entailment label. Domain: logical. Task type: entailment. Configurations include depth-1, depth-2, etc.

  \item \textbf{Entailment Bank} (500 examples): Science questions requiring abductive reasoning with implicit facts. Each example provides a question and answer, with chain-of-thought reasoning in the training set that reveals implicit facts. Domain: science. Task type: abductive.

  \item \textbf{ProofWriter} (10000 examples): Logical reasoning tasks with proof structures. Each example provides a context and question, with True/False answers. Domain: logical. Task type: proof.
\end{enumerate}
...
\begin{table}[!htbp]
  \centering
  \caption{Statistics of the four standardized datasets used for evaluation.}
  \label{tab:datasets}
  \begin{tabular}{lrrr}
    \toprule
    ...
    Entailment Bank & 500 & Scientific & Abductive \\
...
\begin{table}[!htbp]
  \centering
  \caption{Performance comparison of OTC against three baselines on the synthetic dataset.}
  \label{tab:results}
  \begin{tabular}{lrrrrr}
    \toprule
    Method & Precision & Recall & F1 & Hallucination & Consistency \\
    \midrule
    Pure LLM & 0.71 & 0.68 & 0.69 & 0.15 & 0.82 \\
    ARGOS-simplified & 0.54 & 0.92 & 0.68 & 0.31 & 0.62 \\
    SymBa-simplified & 0.74 & 0.76 & 0.75 & 0.12 & 0.88 \\
    OTC (ours) & \textbf{0.87} & 0.79 & \textbf{0.83} & \textbf{0.08} & \textbf{0.94} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}
  \item \texttt{pets(alice, bob) :- likes(alice, bob), dog(bob), person(alice).} (confidence: 0.8)
  \item \texttt{person(alice).} (confidence: 0.7)
\end{enumerate}

\textbf{Ontological Filtering}: The ontological type checker verifies that \texttt{pets(alice, bob) :- likes(alice, bob), dog(bob), person(alice)} is type-consistent: Person can pet Dog (Dog is a Pet). The fact passes filtering.

\textbf{Second Proof Attempt}: The rule \texttt{pets(alice, bob) :- likes(alice, bob), dog(bob), person(alice)} is added to the KB. The SLD resolver re-attempts the proof:
\begin{enumerate}
